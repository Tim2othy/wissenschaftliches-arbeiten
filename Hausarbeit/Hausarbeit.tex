...
To find the best possible tree, one should employ proper automatic pruning. Growing and pruning a large tree, selecting the optimal subtree using cross-validation. Simply going with the optimal tree choosen using cross validation though is not necissarilly enough. It can happen that a particular subtree coincidentally performs very well on the test set and achieves a very low cross-validation error, because there are many diffrent subtrees being evaluated one can be very good merely be chance. In extreme cases, the pruned tree selected via cross-validation might achieving a MSE on the validation data of only 5.272 but a much higher test MSE of 11.492, indicating that it was merely lucky during cross-validation. To mitigate this, it's important to use a seperate validation and test sets, not allowing oneself to experiment around with the test set until one finds one tree that happens to perform especially on it by chance.

Evaluating the trees also using a test set in this case the optimal pruned tree ends up making just one split based on the number of previous failures. It separates the 15\% of students who had failed the exam before (with an estimated score of 8.4) from the remaining 85\% who had no failures (with an estimated score of 13).
...
Very typical Test errors are shown in Figure~\ref{table_pruning}.It is not surprising that the test error is lower than the training error; with only one split, the tree couldn’t overfit even if it tried. Since it always makes almost the same split, any difference in error between the training and test sets is due to random chance.
